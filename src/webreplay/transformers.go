...
// getCSPScriptSrcDirectivesFromHeaders returns a Content-Security-Policy (CSP)
...
// getCSPScriptSrcDirectivesFromHeaders returns an empty string.
func getCSPScriptSrcDirectivesFromHeaders(header http.Header) []string {
	csps := header.Values("Content-Security-Policy")
	directives := []string{}

	for _, csp := range csps {
		// Some sites will have more than one csp entry
		directives = append(directives, getCSPScriptSrcDirective(csp))
	}

	return directives
}

func getCSPScriptSrcDirective(csp string) string {
	if csp == "" {
		return ""
	}
...
func (si *scriptInjector) Transform(req *http.Request, resp *http.Response) {
	...
	// Skip requests targetting different domains
	if !ShouldIncludeTransformation(req, si.filename) {
		return
	}
	...
	transformResponseBody(resp, func(body []byte) []byte {
		...
		for _, directive := range getCSPScriptSrcDirectivesFromHeaders(resp.Header) {
			if directive == "" {
				continue
			}
